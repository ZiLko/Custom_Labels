...
bool TimeLabelSettings::setup() {
    ...
    attemptTime = lb.labels[labelIndex].attemptTime;
    sessionTime = lb.labels[labelIndex].sessionTime;
    ...
    auto label = CCLabelBMFont::create("Attempt time", "bigFont.fnt");
    label->setScale(0.3f);
    label->setPosition(winSize / 2 + ccp(44, 63));
    m_mainLayer->addChild(label);

    attemptTimeToggle = CCMenuItemToggler::create(
        checkOffSprite,
        checkOnSprite,
        this,
        menu_selector(TimeLabelSettings::toggleAttemptTime)
    );

    attemptTimeToggle->setPosition(winSize / 2 + ccp(44, 86));
    attemptTimeToggle->setScale(0.85f);
    attemptTimeToggle->toggle(attemptTime);
    menu->addChild(attemptTimeToggle);

    label = CCLabelBMFont::create("Session Time", "bigFont.fnt");
    label->setScale(0.3f);
    label->setPosition(winSize / 2 + ccp(134, 63));
    m_mainLayer->addChild(label);

    sessionTimeToggle = CCMenuItemToggler::create(
        checkOffSprite,
        checkOnSprite,
        this,
        menu_selector(TimeLabelSettings::toggleSessionTime)
    );

    sessionTimeToggle->setPosition(winSize / 2 + ccp(134, 86));
    sessionTimeToggle->setScale(0.85f);
    sessionTimeToggle->toggle(sessionTime);
    menu->addChild(sessionTimeToggle);
    ...
    auto btn = CCMenuItemSpriteExtra::create(
        btnSpr,
        this,
        menu_selector(TimeLabelSettings::saveSettings)
    );
    ...
    btn = CCMenuItemSpriteExtra::create(
        colorSprite,
        this,
        menu_selector(TimeLabelSettings::openColorPicker)
    );
    ...
    btn = CCMenuItemSpriteExtra::create(
        spr,
        this,
        menu_selector(TimeLabelSettings::switchPos)
    );
    btn->setID("right");
    ...
    btn = CCMenuItemSpriteExtra::create(
        spr,
        this,
        menu_selector(TimeLabelSettings::switchFont)
    );
    btn->setID("right");
    ...
    opacitySlider = Slider::create(
        this,
        menu_selector(TimeLabelSettings::updateOpacity),
        0.8f
    );
    ...
    sizeSlider = Slider::create(
        this,
        menu_selector(TimeLabelSettings::updateSize),
        0.8f
    );
    ...
    offsetXSlider = Slider::create(
        this,
        menu_selector(TimeLabelSettings::updateOffsetX),
        0.8f
    );
    ...
    offsetYSlider = Slider::create(
        this,
        menu_selector(TimeLabelSettings::updateOffsetY),
        0.8f
    );
...
TimeLabelSettings* TimeLabelSettings::create(int labelIndex) {
    auto ret = new TimeLabelSettings();
...
void TimeLabelSettings::switchFont(CCObject* obj) {
    auto id = static_cast<CCNode*>(obj)->getID();
    auto& lb = Labels::get();
...
void TimeLabelSettings::saveSettings(CCObject*) {
    keyBackClicked();
    auto& lb = Labels::get();
    ...
    lb.labels[labelIndex].attemptTime = attemptTime;
    lb.labels[labelIndex].sessionTime = sessionTime;
...
void TimeLabelSettings::openColorPicker(CCObject*) {
    auto popup = ColorPickPopup::create(colorSprite->getColor());
    popup->setColorTarget(colorSprite);
    popup->show();
}

void TimeLabelSettings::toggleAttemptTime(CCObject*) {
    if (attemptTime) {
        attemptTimeToggle->toggle(false);
    } else {
        attemptTime = true;
        sessionTimeToggle->toggle(false);
        sessionTime = false;
    }
}

void TimeLabelSettings::toggleSessionTime(CCObject*) {
    if (sessionTime) {
        sessionTimeToggle->toggle(false);
    } else {
        sessionTime = true;
