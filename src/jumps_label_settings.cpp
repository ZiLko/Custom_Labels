...
bool JumpsLabelSettings::setup() {
    ...
    attemptJumps = lb.labels[labelIndex].attemptJumps;
    sessionJumps = lb.labels[labelIndex].sessionJumps;
    ...
    auto label = CCLabelBMFont::create("Attempt Jumps", "bigFont.fnt");
    label->setScale(0.3f);
    label->setPosition(winSize / 2 + ccp(44, 63));
    m_mainLayer->addChild(label);

    attemptJumpsToggle = CCMenuItemToggler::create(
        checkOffSprite,
        checkOnSprite,
        this,
        menu_selector(JumpsLabelSettings::toggleAttemptJumps)
    );

    attemptJumpsToggle->setPosition(winSize / 2 + ccp(44, 86));
    attemptJumpsToggle->setScale(0.85f);
    attemptJumpsToggle->toggle(attemptJumps);
    menu->addChild(attemptJumpsToggle);

    label = CCLabelBMFont::create("Session Jumps", "bigFont.fnt");
    label->setScale(0.3f);
    label->setPosition(winSize / 2 + ccp(134, 63));
    m_mainLayer->addChild(label);

    sessionJumpsToggle = CCMenuItemToggler::create(
        checkOffSprite,
        checkOnSprite,
        this,
        menu_selector(JumpsLabelSettings::toggleSessionJumps)
    );

    sessionJumpsToggle->setPosition(winSize / 2 + ccp(134, 86));
    sessionJumpsToggle->setScale(0.85f);
    sessionJumpsToggle->toggle(sessionJumps);
    menu->addChild(sessionJumpsToggle);
    ...
    auto btn = CCMenuItemSpriteExtra::create(
        btnSpr,
        this,
        menu_selector(JumpsLabelSettings::saveSettings)
    );
    ...
    btn = CCMenuItemSpriteExtra::create(
        colorSprite,
        this,
        menu_selector(JumpsLabelSettings::openColorPicker)
    );
    ...
    btn = CCMenuItemSpriteExtra::create(
        spr,
        this,
        menu_selector(JumpsLabelSettings::switchPos)
    );
    btn->setID("right");
    ...
    btn = CCMenuItemSpriteExtra::create(
        spr,
        this,
        menu_selector(JumpsLabelSettings::switchFont)
    );
    btn->setID("right");
    ...
    opacitySlider = Slider::create(
        this,
        menu_selector(JumpsLabelSettings::updateOpacity),
        0.8f
    );
    ...
    sizeSlider = Slider::create(
        this,
        menu_selector(JumpsLabelSettings::updateSize),
        0.8f
    );
    ...
    offsetXSlider = Slider::create(
        this,
        menu_selector(JumpsLabelSettings::updateOffsetX),
        0.8f
    );
    ...
    offsetYSlider = Slider::create(
        this,
        menu_selector(JumpsLabelSettings::updateOffsetY),
        0.8f
    );
...
JumpsLabelSettings* JumpsLabelSettings::create(int labelIndex) {
    auto ret = new JumpsLabelSettings();
...
void JumpsLabelSettings::switchFont(CCObject* obj) {
    auto id = static_cast<CCNode*>(obj)->getID();
    auto& lb = Labels::get();
...
void JumpsLabelSettings::saveSettings(CCObject*) {
    keyBackClicked();
    auto& lb = Labels::get();
    ...
    lb.labels[labelIndex].attemptJumps = attemptJumps;
    lb.labels[labelIndex].sessionJumps = sessionJumps;
...
void JumpsLabelSettings::openColorPicker(CCObject*) {
    auto popup = ColorPickPopup::create(colorSprite->getColor());
    popup->setColorTarget(colorSprite);
    popup->show();
}

void JumpsLabelSettings::toggleAttemptJumps(CCObject*) {
    if (attemptJumps) {
        attemptJumpsToggle->toggle(false);
    } else {
        attemptJumps = true;
        sessionJumpsToggle->toggle(false);
        sessionJumps = false;
    }
}

void JumpsLabelSettings::toggleSessionJumps(CCObject*) {
    if (sessionJumps) {
        sessionJumpsToggle->toggle(false);
    } else {
        sessionJumps = true;
